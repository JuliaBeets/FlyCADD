...
The first step in creating the FlyCADD was obtaining the reconstructed ancestral sequence. The scripts in this folder were used to extract the ancestral sequence from the Multi-species Sequence Alignment (MSA)  from B. Kim et al. (2024). The alignment was generated with Cactus and includes ~ 300 Drosophila species and an reconstructed ancestral sequence for each node in the phylogeny in `.maf` format. Species names used in these scripts should be written as they are in the MSA.
