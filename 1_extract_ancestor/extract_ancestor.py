...
This script takes as input the preprecessed maf files,
...
def sequence_processing(g_seq,ancestor_seq):
	...
	# This way the true length of the ancestor can be identified and it will fill up all the remaining positions with gaps. #changed ancestor_seq.annotations to g_seq.annotations at start
...
#PIG lines = [ x for x in lines if "AEMK" not in x ]
lines = [line for line in lines if len(line.split()[0]) <= 3]
for line in lines:
	chrom = line.split()[0]
	length = line.split()[1]
...
for alignment in alignment_generator:
	...
	dros_seq = ancestor_finder(alignment,sc_name)
	...
	if (not isinstance(anc_seq,SeqIO.SeqRecord)) or (str(sc_name) +'.' not in sp_seq.id):
		...
		dros_start = dros_seq.annotations['start']
		if dros_start in anc_pos_seq_dict:
			curr_seq = anc_seq_processed.seq
			other_seq = anc_pos_seq_dict[dros_start].seq
			updated_seq = ''
			...
			updated_seq_record = copy.deepcopy(anc_pos_seq_dict[dros_start])
			updated_seq_record.seq = Seq(updated_seq)
			anc_pos_seq_dict[dros_start] = updated_seq_record

		else:
			anc_pos_seq_dict[dros_start] = anc_seq_processed
...
for i,position in enumerate(list(pos_list_s)[1:]):
	# The start of the previous position needs to be bigger then the ancestor_record
	if position+1 < len(ancestor_record):
		print('The start position of ' + str(position) + ' is located in the previously added sub sequence that is already appended to the Ancestral sequence!')
