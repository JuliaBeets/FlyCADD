...
parser.add_option("-v","--vep", dest="vep", help="VEP (variant effect predictor) output",default="./derived_chr19_VEP-annotated.vcf")
parser.add_option("-r","--reference", dest="reference", help="Path to reference chr",default='./chr19.fna')
parser.add_option("-s","--vcf_source", dest="vcf", help="Path to bgzipped & tabix index vcf source file (vcf file of the generated variants before VEP annotation)",default="./snps_derived_var_chr_19_case_upper.vcf.gz") 

parser.add_option("-g","--grantham", dest="grantham", help="Path to Grantham score annotation file",default='./grantham_matrix_formatted_correct.tsv')
...
outp_chr = outp_chr.split('/ref')[-1]
if 'sim' in options.vcf:
	variant_type = 'simulated'
elif 'derived' in options.vcf:
	variant_type = 'derived'
outp = open(variant_type + '_vep_processed_chr' + outp_chr + '.vcf', 'w')
