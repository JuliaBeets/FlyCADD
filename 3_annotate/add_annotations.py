...
:Example:
python add_annotationsV4.py -o <Path to merged annotations> -a <Path to annotation to add> -b <Path to second annotation to add> -c <Path to third annotation to add> -d <Path to fourth annotation to add> etc
...
for filen in annotated_list:
	...
	if 'derived' in filen:
		type_f = 'derived variants'
	elif 'simulated' in filen: 
		type_f = 'simulated variants'

	# Determine chr number. 
	chr_num = filen.split('_')[0]
	print('Working on: Chr' + chr_num + ', ' + type_f)
	...
	if 'derived' in filen:
		df_merged.to_csv(str(chr_num) + '_derived_merged_features.tsv', index=None, sep='\t')
	elif 'simulated' in filen:
		df_merged.to_csv(str(chr_num) + '_simulated_merged_features.tsv', index=None, sep='\t')