...
This script takes fully annotated variant files for simulated and derived variants respectively. 
...
:Edited by: Seyan Hu
:Date: 13-12-2022
:Usage: python data_encoding_mv_handling.py -i <Merged annotation file> (add '-d' if this is performed on derived variants)
...
parser.add_option("-d","--derived", dest="derived", help="Flag, if given it will relabel Ref and Alt. (def OFF)",default=False,action="store_true")
...
#myData['SIFTcat'] = myData['SIFTcat'].map({'tolerated':'tolerated', 'tolerated_low_confidence':'unknown','deleterious':'deleterious','deleterious_low_confidence':'unknown',np.nan:'UD'})
...
# (Generate &) Load the mean imputations fromt he simulated variants. 
...
if options.derived==False:
	try:
		with open(chr_num + '_imputation_dict.txt') as f: 
			means = ast.literal_eval(f.read())
	except IOError:
		for label in selection:
			means[label] = myData[label].mean()
		f = open(chr_num + '_imputation_dict.txt',"w")
		f.write(str(means))
		f.close()
	...
	sys.exit("It is neither simulated or derived therefore an error message occurs here.")
...
# Renamed Ref/Alt and nAA/oAA, renaming the Consequences Stop gained and Stop lost is easier after the OneHotEncoding therefore this is performed later.
if options.derived==True:
	columnsTitles={'Alt':'Ref', 'Ref':'Alt','nAA':'oAA','oAA':'nAA'}
	myData.rename(index=str, columns=columnsTitles,inplace=True)
...
# After endcoding of the joined basic features, rename Stop gained and Stop lost in the derived data set before joining both sets.
if options.derived==True:
	columnsTitles={'Consequence_SG':'Consequence_SL', 'Consequence_SL':'Consequence_SG'}
	myData.rename(index=str, columns=columnsTitles,inplace=True)
...
# Add data to the encoded features. 
# First creating a new empty dataframe "Alt1" which is filled and eventually concatenate myData and Alt1. 
# In this way you would avoid copying over and over again myData just to add 1 column.
...
if options.derived==True:
	myData.to_csv(chr_num + '_derived_enc_mv.csv', index=False,na_rep='-')
else:
	myData.to_csv(chr_num + '_simulated_enc_mv.csv', index=False,na_rep='-')