...
for filen in VEP_list:
	...
	if 'derived' in filen:
		df_vep.insert(loc=0, column='#Derived(1)_Simulated(0)', value=1)
		type_f = 'derived variants'
	elif 'simulated' in filen: 
		df_vep.insert(loc=0, column='#Derived(1)_Simulated(0)', value=0)
		type_f = 'simulated variants'
	print(df_vep.head(3))
	
	# Determine chr munber. 
	chr_num = filen.split('chr')[-1]
	chr_num = chr_num.replace(".vcf", "")
